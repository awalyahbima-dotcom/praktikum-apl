...
void sortMerkAsc(Kendaraan *k, int n){
    for(int i=0;i<n;i++){
        for(int j=i+1;j<n;j++){
            if(k[i].dataMobil.merkMobil > k[j].dataMobil.merkMobil){
                swap(k[i], k[j]);
            }
        }
    }
    cout<<"Data sudah diurutkan berdasarkan merk\n";
}

void sortTahunDesc(Kendaraan *k, int n){
    for(int i=0;i<n-1;i++){
        for(int j=n-1;j>i;j--){
            if(k[j].tahunMobil > k[j-1].tahunMobil){
                swap(k[j], k[j-1]);
            }
        }
    }
    cout<<"Data sudah diurutkan berdasarkan tahun\n";
}

void sortPlatAsc(Kendaraan *k, int n){
    int i=0;
    while(i<n-1){
        int j=0;
        while(j<n-i-1){
            if(k[j].noPlat > k[j+1].noPlat){
                swap(k[j], k[j+1]);
            }
            j++;
        }
    ...
    cout<<"Data sudah diurutkan berdasarkan plat\n";
}
                ...
                if(cekUser(nama,nim,akunData,jumlahUser)){
                    ...
                    do{
                        ...
                        if(menu==1){
                            ...
                            sortMerkAsc(mobilData, jumlahData);
                        }

                        else if(menu==9){
                            sortTahunDesc(mobilData, jumlahData);
                        }

                        else if(menu==10){
                            sortPlatAsc(mobilData, jumlahData);
                        }
